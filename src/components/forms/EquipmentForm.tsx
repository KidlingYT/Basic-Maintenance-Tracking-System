...
const equipmentSchema = z.object({
  ...
  serialNumber: z.string().regex(/^[a-z0-9]+$/i).min(1, "Serial Number is required"),
    ...
    <>
      {isSuccessMessage && (
        <div className="fixed top-10 right-10 bg-green-500 text-white p-4 rounded shadow-lg">
          <p className="success-message">Equipment successfully created!</p>
        </div>
      )}
      {!isSuccessMessage && (
      <div
        className="z-0 fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 dark:bg-opacity-80 dark:bg-neutral-900"
        onClick={onClose}
      >
        <form
        ...
        >
          ...
          <label className="p-2">
            ...
            {errors.name && <p className="text-red-500 name-error">{errors.name.message}</p>}
          </label>
          <label className="p-2">
            Location:
            <input {...register("location")} className="border p-1 w-full" />
            {errors.location && <p className="text-red-500 location-error">{errors.location.message}</p>}
          </label>
          <label className="p-2">
            Department:
          ...
          <label className="p-2">
            ...
            {errors.model && <p className="text-red-500 model-error">{errors.model.message}</p>}
          </label>
          <label className="p-2">
            Serial Number:
            <input {...register("serialNumber")} className="border p-1 w-full" />
            {errors.serialNumber && <p className="text-red-500 serialNumber-error">{errors.serialNumber.message}</p>}
          </label>
          <label className="p-2">
            Install Date:
            <input
            ...
            />
            {errors.installDate && (
              <p className="text-red-500 installDate-error">Date must be in the past</p>
            )}
          </label>
          <label className="p-2">
            Status:
